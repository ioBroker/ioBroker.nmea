...
export default function Rudder(props: {
    rudder: number | null;
    minMax?: number;
    zoomAt: number;
    zoomDelay: number;
    ...
    const [zoomActive, setZoomActive] = useState(Math.abs(props.rudder || 0) <= props.zoomAt);
    ...
    const minMax = zoomActive
        ? props.zoomAt + (props.zoomDelay || 4)
        : typeof props.minMax === 'number'
          ? Math.abs(props.minMax)
          : 50;
    ...
    const compass = useMemo(
        () => (
            <>
                ...
                <Lines
                    ...
                    textCallback={_angle => (_angle % 10 ? '' : _angle > 0 ? _angle : -_angle)}
                    paddingCallback={_angle => (Math.abs(_angle) % 10 ? 0 : 20)}
                    getAngles={() => {
                        const angles = [];
        ...
        [props.themeType, zoomActive, factor],
    );

    return (
        <div style={styles.content}>
            <div style={styles.header}>{Generic.t('Rudder')}</div>
            <div style={styles.contentInner}>
                <IconButton
                    ...
                    disabled={
                        zoomActive
                            ? Math.abs(props.rudder || 0) <= props.zoomAt - (props.zoomDelay || 4)
                            : Math.abs(props.rudder || 0) > props.zoomAt + (props.zoomDelay || 4)
                    }
                    style={{
                        position: 'absolute',
                        ...
                        zIndex: 1,
                        display:
                            !zoomActive && Math.abs(props.rudder || 0) > props.zoomAt + (props.zoomDelay || 4)
                                ? 'none'
                                : 'block',
                    }}
                >
                ...
                {/* Arrow */}
                {props.rudder !== null && props.rudder !== undefined ? (
                    <SvgContainer
