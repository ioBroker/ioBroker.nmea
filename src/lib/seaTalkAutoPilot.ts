...
class SeaTalkAutoPilot {
    ...
    setLockedHeading(angle: number): void {
        ...
        const data = encodeActisense({ prio: 3, pgn: 126208, src: 7, dst: this.autoPilotAddress, data: Buffer.from([0x01,0x50,0xff,0x00,0xf8,0x03,0x01,0x3b,0x07,0x03,0x04,0x06,a & 0xFF, (a >> 8) & 0xFF]) });
        this.nmeaDriver.write(data);
    }
    // decrement 1:  0x05FA
    // decrement 10: 0x06F9
